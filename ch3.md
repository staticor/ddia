...
现在我们可以对段文件的格式做一个简单的改变：要求键值对的序列按键排序。乍一看，这个要求看上去违背了前面只能使用顺序写入的规则，我们将稍后再回到这个问题。
...
![](/Applications/Gridea.app/Contents/Resources/app/img/ddiafig3-6.png)
...
正如 [第二章](ch2.md) 所探讨的，根据应用程序的需要，在事务处理领域中使用了大量不同的数据模型。另一方面，在分析型业务中，数据模型的多样性则少得多。许多数据仓库都以公式化的方式设计出来，比较著名的设计"套路"被称为星型建模（也称为维度建模【55】）。
